...
class Wheelchair {
  ...
  private:
    ...
    float D_MASSCENTER = 0.11; 
    float I_BODY = 3.26; // Ic+mcd^2+2mwL^2+2Im, 5.16 + 4.5~5.0(behind term)
    float M_BODY = 42.2; // m = mc+2mw -> 34.4+7.84 = 42.2
    ...
    void _pid_control();
    void _pid_reset();
    void g_compensation_control(float phi_ref, float theta, float *left, float *right); // compensate the gravity
    ...
    float* DesiredMotion(float loadcell_current[], float loadcell_current_hand[], float _rpm[]);
    float* TrackingPid(float *desired_acc, float _rpm[], float _accel[]);
    void SearchParameter(float loadcell_current[], float loadcell_current_hand[], float _rpm[], float _accel[]);
    ...
    int LeftTouch();      //JW
    bool MiddleTouch();      //JW
    bool MiddleTouchWeak();
    int SIT_DOWNTouch();      //JW
    void FOLD();      //JW
    int LA_HIGH_COUNT = 0;       //JW
    int LA_LOW_COUNT = 0;       //JW
