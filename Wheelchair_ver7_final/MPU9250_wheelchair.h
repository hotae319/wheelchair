...
int j_reset = 0; float fq_test = 100; // for reset setting
...
#define Kp_mpu 2.0f * 5.0f // these are the free parameters in the Mahony filter and fusion scheme, Kp for proportional feedback, Ki for integral
#define Ki_mpu 0.0f

uint32_t delt_t = 0; // used to control display output rate
uint32_t count = 0, sumCount = 0; // used to control display output rate
float pitch, yaw, roll;
float comp_pitch, comp_roll, comp_theta, comp_phi_ref, comp_yaw;
...
bool print_flag = false; // for print at the same time as imu print  
//===================================================================================================================
...
void MadgwickQuaternionUpdate_6050(float ax, float ay, float az, float gx, float gy, float gz)
        {
...
void compensation_filter(float ax, float ay, float az, float gx, float gy, float gz){
   static float gyro_roll=0, gyro_pitch=0, gyro_phi_ref=0, gyro_yaw = 0;
   static float acc_roll, acc_pitch, acc_phi_ref, mag_yaw;
   ...
   mag_yaw = atan2(-my,mx)/PI*180.0f;

   if(count == 0){
    comp_roll = acc_roll;
    comp_pitch = acc_pitch;
    comp_phi_ref = acc_phi_ref;
    comp_yaw = mag_yaw;
    count ++;
   }

   // gyroscope
   gyro_roll = comp_roll + gx*dt;
   gyro_pitch = comp_pitch + gy*dt;   
   gyro_phi_ref = comp_phi_ref*180.0f/PI + gz*dt;
   gyro_yaw = comp_yaw + gz*dt;
   ...
   gyro_yaw = gyro_yaw - int((gyro_yaw+180.0)/360.0)*360.0;
   //atan(sqrtf(ax*ax+ay*ay)/abs(az));
   // complementary
   comp_roll = alpha * gyro_roll + (1-alpha) * acc_roll;
   comp_pitch = alpha * gyro_pitch + (1-alpha) * acc_pitch;
   comp_phi_ref = alpha * gyro_phi_ref + (1-alpha) * acc_phi_ref*180.0f/PI;
   comp_yaw = alpha * gyro_yaw + (1-alpha) * mag_yaw;
}
...
void mpu9250_wheelchair_act(float accel0, float accel1, float rpm0, float rpm1, int state){
  ...
  if(j==0){
    ...
    //Serial.print(1000*ax); Serial.print(1000*ay); Serial.print(1000*az);
    ...
    magbias[1] = -180.;  // User environmental x-axis correction in milliGauss
    magbias[2] = -75.;  // User environmental x-axis correction in milliGauss
...
MadgwickQuaternionUpdate_6050(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f);
    ...
    axr = 9.81*ax - RADIUS_WHEEL*(accel0+accel1)/2.0 - 0.05*(gz/180.0f*PI)*(gz/180.0f*PI)*sgn_mpu(gz);
    axr = axr/9.81;
    ...
    // idling
        ...
        axr = 9.81*ax - 0.03*(gz-pre_gz)/180.0f*PI/delt*1000.0f+(0.05*(gz-pre_gz)/180.0f*PI/delt*1000.0f-0.04*(gz/180.0f*PI)*(gz/180.0f*PI)*sgn_mpu(gz));
        axr = axr/9.81;
        ayr = 9.81*ay -0.03*(gz/180.0f*PI)*(gz/180.0f*PI) + (0.05*(gz/180.0f*PI)*(gz/180.0f*PI)*sgn_mpu(gz) + 0.04*(gz-pre_gz)/180.0f*PI/delt*1000.0f);
        ayr = ayr/9.81;
    }
    // rotating
    else if(abs(rpm0+rpm1)<50 && abs(gz-pre_gz)>2){
      ayr = 9.81*ay - 0.04*(gz-pre_gz)/180.0f*PI/delt*1000.0f - 0.05*(gz/180.0f*PI)*(gz/180.0f*PI)*sgn_mpu(gz)- (rpm1+rpm0)/2.0/60.0*2.0*PI*RADIUS_WHEEL*(gz/180.0f*PI);; 
      ayr = ayr/9.81;
    }
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    w = sin(yaw)*sin(pitch)*sin(roll) + cos(yaw)*cos(roll);  
    phi_ref = atan2(-ay,-ax); // we consider the inclination and the angle from gsin direction
    // we only consider theta in [0,pi/2]
    theta = acos(cos(pitch)*cos(roll));
    // all rotation sum
    phi = -atan2((z-y)/(1+cos(theta)),(x+w)/(1+cos(theta)));

    // we only consider theta in [0,pi/2]
 //phi = atan2(-sin(pitch)/sin(theta),cos(pitch)*sin(roll)/sin(theta));
    if(-sin(pitch)/sin(theta)>0){ // if cos>0  
      if(cos(pitch)*sin(roll)/sin(theta)>0){ // if sin>0
        phi_s = asin(sin(roll)*cos(pitch)/sin(theta));
        phi_c = acos(-sin(pitch)/sin(theta));
      }
      else{
        phi_s = asin(sin(roll)*cos(pitch)/sin(theta));
        phi_c = -acos(-sin(pitch)/sin(theta));
      ...
      if(cos(pitch)*sin(roll)/sin(theta)>0){
        phi_s = PI - asin(sin(roll)*cos(pitch)/sin(theta));
        phi_c = acos(-sin(pitch)/sin(theta));
      }
      else{
        phi_s = -PI - asin(sin(roll)*cos(pitch)/sin(theta));
